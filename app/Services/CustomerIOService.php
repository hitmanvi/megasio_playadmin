...
class CustomerIOService
{
    ...
    /**
     * 将用户「是否接收营销邮件」同步到 Customer.io（customer id 为 uid）。
     * receive_promotion_email 与 unsubscribed 相反：接收营销 → unsubscribed=false；不接收 → unsubscribed=true。
     */
    public function syncReceivePromotionEmail(User $user): void
    {
        $receive = (bool) $user->receive_promotion_email;
        $this->update($user, [
            'unsubscribed' => ! $receive,
        ]);
    }
